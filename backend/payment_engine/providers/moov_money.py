"""Provider Moov Money (Mobile Money, Togo).

Meme logique que YASTMoneyProvider : simulation par defaut au J0, adaptateur
isole et remplacable, aucune dependance vers un autre provider.
"""
...
class MoovMoneyProvider(PaymentProvider):
    ...
    def initiate_payment(
        ...
        transaction_ref = f"MOOV-{order_id}-{uuid.uuid4().hex[:8]}"
        ...
        return PaymentResult(
            ...
            montant=amount,
            devise=currency,
            message="Transaction simulee Moov Money." if self.simulate else "Paiement initie.",
        )
